...
class TestArticulatedRobot(unittest.TestCase):
    ...
    def test_inverse_reachable(self):
        ...
        thetas = r.inverse_kinematics(target_orientation=t_goal[:3,:3], tolerance_threshold=0.01, restart_threshold=1000)

        # Test with forward kinematics
        calc_pos = r.forward_kinematics(thetas)
        round_pos = np.round(calc_pos, 1)

        np.testing.assert_allclose(round_pos, t_goal)
